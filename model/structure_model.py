...
# from torch_geometric.nn.pool.connect import filter_adj
...
class NodeInformationScore(MessagePassing):
    ...
    def norm(edge_index, num_nodes, edge_weight, dtype=None):
        ...
        # print(expand_deg.shape,deg_inv_sqrt[row].shape,edge_weight.shape,deg_inv_sqrt[col].shape)
...
class HGPSLPool(torch.nn.Module):
    def __init__(self, in_channels, ratio=0.8, sample=False, sparse=False, sl=True, lamb=1.0, negative_slop=0.2):
        ...
        self.topk = SelectTopK(self.in_channels,self.ratio)
        ...
        self.perm_embedding = nn.Linear(2,1)
        # self.perm_embedding2 = torch.nn.Embedding(2, 1)
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        if edge_attr!=None:
            ...
            if len(edge_attr.shape)==2:
                edge_attr = self.perm_embedding(edge_attr.to(dtype=torch.float32))
        ...
        # print(perm)
        topkres = self.topk(x,batch)
        perm = topkres.node_index
        score = topkres.weight
        # print(x.shape)
        x = x[perm]
        # print(x.shape)
        batch = batch[perm]
        ...
        if self.sample:
            ...
            new_edge_index, new_edge_attr = filter_adj(hop_edge_index, hop_edge_attr, perm)

            new_edge_index, new_edge_attr = add_remaining_self_loops(new_edge_index, new_edge_attr, 0, x.size(0))
            row, col = new_edge_index
            weights = (torch.cat([x[row], x[col]], dim=1) * self.att).sum(dim=-1)
            
            # print(weights.shape,new_edge_attr.shape)
