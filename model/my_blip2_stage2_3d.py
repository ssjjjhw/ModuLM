...
import torch
from model.my_blip2_opt3d import Blip2OPT
...
class Blip2Stage2(pl.LightningModule):
    ...
    def save_predictions_valid(self, predictions, targets):
        ...
        # with open(os.path.join('/data/cz/moltc/results/3dresults/fanzhuan/newgin/', f'CHCHtestreverse_epoch_{self.current_epoch}_{self.args.num_query_token}.txt'), 'w', encoding='utf8') as f:
        output_dir = f'/home/cz/MolTC-main/result/{self.args.backbone}/ChChMiner4d/{self.args.llm_tune}'
        os.makedirs(output_dir, exist_ok=True)
        file_path = os.path.join(output_dir, f'predictions_{self.current_epoch}_{self.args.num_query_token}.txt')
        # with open(os.path.join(f'/work/home/ac15y5ara4/cz/MolTC-main/result/deepseek/deepseek1.5b/ChChMiner/{self.args.llm_tune}', f'predictions_{self.current_epoch}_{self.args.num_query_token}.txt'), 'w', encoding='utf8') as f:
    ...
    def save_predictions_valid_autozijiegou(self, predictions, targets, mlppredicts):
        assert len(predictions) == len(targets) == len(mlppredicts)
        print("----------------")
        print(len(predictions))
        print("----------------")
        # with open(os.path.join('/work/home/ac15y5ara4/cz/MolTC-main/result/deepseek/', 'newtest.txt'), 'w', encoding='utf8') as f:
        #     for p, t, m in zip(predictions, targets, mlppredicts):
        #         line = {'prediction': p, 'target': t, 'mlppredict': m.item()}
        #         f.write(json.dumps(line, ensure_ascii=True) + '\n')
        output_dir = f'/home/cz/MolTC-main/result/deepseek/deepseek1.5b/Combisolv/{self.args.llm_tune}'
        os.makedirs(output_dir, exist_ok=True)
        file_path = os.path.join(output_dir, f'predictions_{self.current_epoch}_{self.args.num_query_token}.txt')
        # with open(os.path.join(f'/work/home/ac15y5ara4/cz/MolTC-main/result/deepseek/deepseek1.5b/ChChMiner/{self.args.llm_tune}', f'predictions_{self.current_epoch}_{self.args.num_query_token}.txt'), 'w', encoding='utf8') as f:
        with open(file_path, 'w', encoding='utf8') as f:  
        ...
        # if self.args.solve == True:
            # pass
            # mae,Rmse=maeRmse("/work/home/ac15y5ara4/cz/MolTC-main/result/deepseek/huiguitest.txt")
            # with open("/work/home/ac15y5ara4/cz/MolTC-main/result/deepseek/huiguibest.txt", 'a', encoding='utf8') as f:
