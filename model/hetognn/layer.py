...
class multi_shallow_embedding_heto(nn.Module):
    def __init__(self, num_nodes, k_neighs, num_graphs):
        super().__init__()
        
        self.num_nodes = num_nodes  # 这里仍是 2N
        self.k = k_neighs
        self.num_graphs = num_graphs

        self.emb_s = Parameter(Tensor(num_graphs, num_nodes, 1))  # 不变
        self.emb_t = Parameter(Tensor(num_graphs, 1, num_nodes))  # 不变
    ...
    def forward(self, device):
        ...
        # remove class-to-class connections
        half_n = self.num_nodes // 2  # N -> N/2
        adj[:, :half_n, :half_n] = float('-inf')  # 类别 0 到类别 0
        adj[:, half_n:, half_n:] = float('-inf')  # 类别 1 到类别 1
...
class DenseGINConv2d(nn.Module):
    ...
    def forward(self, x: Tensor, adj: Tensor, add_loop=True):
        ...
        # out: [B, C, G, N, F//G]
        # print('denseoutbf',out.shape)
        out = self.mlp(out, True)
        # print('denseoutaf',out.shape)
        # out: [B, C, N, F]
        C = out.size(1)
        out = out.transpose(2, 3).reshape(B, C, N, -1)
        # print('denseoutfin',out.shape)
        return out
