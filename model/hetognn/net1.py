...
class GNNStack(nn.Module):
    ...
    def __init__(self, gnn_model_type, num_layers, groups, pool_ratio, kern_size, 
        ...
        self.g_constr_heto = multi_shallow_embedding_heto(num_nodes, k_neighs, self.num_graphs)
    ...
    def forward(self, inputs: Tensor,heto=False):
        ...
        if heto == False:
            ...
            adj = self.g_constr_heto(x.device)
        # print(adj)
        # print(adj)
        # print('sssss',x.shape)
        for tconv, gconv, bn, pool in zip(self.tconvs, self.gconvs, self.bns, self.diffpool):
            # print('ttttt',tconv(x).shape)
        ...
        # aggregated = self.linear_agg(out).squeeze(-1)
        # print('out1',x.shape)
        aggregated = x.mean(dim=1)
        # print('out2',aggregated.shape)
        # print('addd',out.shape)
        # out = x
        # out = aggregated
